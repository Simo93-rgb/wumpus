...
public class Mappa {
     ...
     * Genera una nuova Mappa di gioco di dimensione latoMappa x latoMappa in cui
     * ogni casella ha una probabilita' del probabilitaVoragine% di contenere una Voragine.
     * La mappa conterra' numTesori elementi Tesoro, numSuperstiti elementi Superstite e
     ...
     * @throws IllegalArgumentException Se latoMappa e' minore di CINQUE o maggiore di 15 caselle;
     *                                  se probabilitaVoragine e' minore di 10 o maggiore di 40;
     *                                  se numTesori e' minore di UNO o maggiore di CINQUE;
     *                                  se numSuperstiti e' minore di UNO o maggiore di CINQUE;
     *                                  se numCuccioli e' minore di UNO o maggiore di CINQUE;
     */
    public Mappa(int latoMappa, int probabilitaVoragine, int numTesori, int numSuperstiti, int numCuccioli) throws IllegalArgumentException {
		...
		*/
        do {
            this.elencoElementiMappa = new Vector<>();
    ...
    protected Mappa(File f) throws IllegalArgumentException, IOException {
        try (BufferedReader bufferedReaderPerCaricamento = new BufferedReader(new FileReader(f))) {
            ...
            bufferedReaderPerCaricamento.readLine();
            ...
            for (int i = 0; i < numeroVoragini; i++) {
                ...
                this.aggiungiAllaMappa(new Voragine(latoCaricato, indRigaVoragine, indColonnaVoragine));
            }
        ...
        for (int i = 0; i < this.getLatoMappa(); i++) {
            ...
            for (int j = 0; j < this.getLatoMappa(); j++) {
				...
				/*
				 ...
				 necessariamente minore di 100, e l'intero 100 darebbe sempre ZERO)
				*/
                if (Math.random() < ((double) probabilitaVoragine / 100)) {
     ...
     * @return <em>true</em> se la casella della Mappa in cui si chiede d'inserire l'Elemento e' libera
     * e l'inserimento va a buon fine, <em>false</em> altrimenti.
     */
    public boolean aggiungiAllaMappa(Elemento daAggiungere) {
        /* La Mappa e' rappresentata attraverso un Vector<Elemento>; per controllare se una casella e' libera
         * e' sufficiente verificare che nel Vector non ci sia gia' un Elemento uguale a quello che si sta
         * chiedendo d'inserire (la equals di Elemento considera uguali due oggetti di tipo Elemento se i
    ...
    public Elemento contenutoCasella(Elemento casellaDaControllare) {
		...
		/*
		 ...
		 e' presente la indexOf ne restituisce l'indice, altrimenti restituisce -1.
		*/
    ...
    */
    private boolean controlloValiditaMappa() {
		...
		*/
        ...
        Vector<Elemento> trovati = new Vector<>();
		...
		*/
        controlloCammini(controfigura, new Vector<>(), trovati);
    ...
    private void controlloCammini(PersonaggioMondoDelWumpus controfigura, Vector<Elemento> caselleGiaRaggiunte, Vector<Elemento> elementiTrovati) {
		/*
		 Si aggiunge la casella corrente al Vector delle caselle gia' raggiunte da una
    ...
    public void salvataggio(String nomeSalvataggio) {
        int contatore;
        try (BufferedWriter salvataggio = new BufferedWriter(new FileWriter(nomeSalvataggio))) {
     ...
     * @author Simone_Garau_20005068
     */
    public static class Voragine extends Elemento {
